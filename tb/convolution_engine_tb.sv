...
initial begin
    ...
    // Use Gaussian kernel (same as phase 2)
    kern_col1 = '{8'sd1, 8'sd2, 8'sd1, 8'sd2, 8'sd4, 8'sd2, 8'sd1, 8'sd2, 8'sd1};
    kern_col2 = '{8'sd1, 8'sd1, 8'sd1, 8'sd1, 8'sd1, 8'sd1, 8'sd1, 8'sd1, 8'sd1};
    ...
    compute_gold_10x10(img_b, kern_col1, kern_col2, gold_b_ch0, gold_b_ch1);

    // Load weights
    for (bi = 0; bi < 9; bi++) begin
        b_wt_pos = bi[3:0]; b_load_en = 1;
        for (int j = 0; j < 32; j++) b_wt_data[j] = kern_col2[bi];
        b_wt_exp = 8'd127;
        #(CLK_PERIOD); #(CLK_PERIOD);
        b_load_en = 0;
        for (int j = 0; j < 32; j++) b_wt_data[j] = kern_col1[bi];
        b_wt_exp = 8'd127;
        #(CLK_PERIOD);
    end
